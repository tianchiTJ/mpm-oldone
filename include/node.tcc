...
void mpm::Node<Tdim, Tdof, Tnphases>::initialise() {
  ...
  internal_force_last_.setZero();
  velocity_.setZero();
  momentum_.setZero();
  acceleration_.setZero();
  damping_force_.setZero();
  status_ = false;
}
...
bool mpm::Node<Tdim, Tdof, Tnphases>::update_internal_force(
  ...
  try {
    ...
    std::lock_guard<std::mutex> guard(node_mutex_);
    internal_force_last_.col(phase) = internal_force_.col(phase);
...
//! Compute the Rayleigh damping force
template <unsigned Tdim, unsigned Tdof, unsigned Tnphases>
bool mpm::Node<Tdim, Tdof, Tnphases>::compute_damping_force(
    unsigned phase, double dt,
    const double rayleigh_alpha, const double rayleigh_beta) {
  bool status = true;
  // Rayleigh damping force
  this->damping_force_ = rayleigh_alpha * this->mass_(phase) * this->velocity_.col(phase) +
                         rayleigh_beta * (this->internal_force_.col(phase) - this->internal_force_last_.col(phase)) / dt;
  return status;
}
  ...
  try {
    if (mass_(phase) > tolerance) {
      ...
      this->acceleration_.col(phase) = (this->external_force_.col(phase) +
                                        this->internal_force_.col(phase) - this->damping_force_) /
                                       this->mass_(phase);
