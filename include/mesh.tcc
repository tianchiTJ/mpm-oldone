...
//ZTC add
template <unsigned Tdim>
bool mpm::Mesh<Tdim>::assign_particle_velocity_constraints(
    const std::vector<std::tuple<mpm::Index, unsigned, double>>&
  ...
  try {
    ...
    for (const auto& particle_velocity_constraint : particle_velocity_constraints) {
      // Particle id
      ...
      //if (map_particles_.find(pid) != map_particles_.end())
        status = map_particles_[pid]->assign_particle_velocity_constraint(dir, velocity);
