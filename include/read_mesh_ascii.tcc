...
//! Return particles velocity particle_constraints
// ZTC add
template <unsigned Tdim>
std::vector<std::tuple<mpm::Index, unsigned, double>>
    mpm::ReadMeshAscii<Tdim>::read_particles_velocity_constraints(
        const std::string& particles_velocity_constraints_file) {
  ...
  // input file stream
  std::fstream file;
  file.open(particles_velocity_constraints_file.c_str(), std::ios::in);
  ...
  try {
    ...
    console_->error("Read particles velocity constraints: {}",
                    exception.what());
    file.close();
  }
  return constraints;
